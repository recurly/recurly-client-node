 ...
 */'use strict';/**
 * LineItemCreate
 * @typedef {Object} LineItemCreate
 * @prop {string} accountingCode - Accounting Code for the `LineItem`. If `item_code`/`item_id` is part of the request then `accounting_code` must be absent.
 ...
 * @prop {string} creditReasonCode - The reason the credit was given when line item is `type=credit`. When the Credit Invoices feature is enabled, the value can be set and will default to `general`. When the Credit Invoices feature is not enabled, the value will always be `null`.
 ...
 * @prop {string} description - Description that appears on the invoice. If `item_code`/`item_id` is part of the request then `description` must be absent.
 * @prop {Date} endDate - If this date is provided, it indicates the end of a time range.
 * @prop {string} itemCode - Unique code to identify an item. Avaliable when the Credit Invoices and Subscription Billing Terms features are enabled.
 ...
 * @prop {Date} startDate - If an end date is present, this is value indicates the beginning of a billing time range. If no end date is present it indicates billing for a specific date. Defaults to the current date-time.
 ...
 * @prop {string} type - Line item type. If `item_code`/`item_id` is present then `type` should not be present. If `item_code`/`item_id` is not present then `type` is required.
 * @prop {number} unitAmount - A positive or negative amount with `type=charge` will result in a positive `unit_amount`. A positive or negative amount with `type=credit` will result in a negative `unit_amount`. If `item_code`/`item_id` is present, `unit_amount` can be passed in, to override the `Item`'s `unit_amount`. If `item_code`/`item_id` is not present then `unit_amount` is required.
 */class LineItemCreate{static getSchema(){return{accountingCode:String,avalaraServiceType:Number,avalaraTransactionType:Number,creditReasonCode:String,currency:String,description:String,endDate:Date,itemCode:String,itemId:String,origin:String,productCode:String,quantity:Number,revenueScheduleType:String,startDate:Date,taxCode:String,taxExempt:Boolean,type:String,unitAmount:Number};}}module.exports=LineItemCreate;