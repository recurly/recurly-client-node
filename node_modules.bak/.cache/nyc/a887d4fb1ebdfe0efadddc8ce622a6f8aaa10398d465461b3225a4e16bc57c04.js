 ...
 */'use strict';/**
 * AccountPurchase
 * @typedef {Object} AccountPurchase
 * @prop {AccountAcquisitionUpdatable} acquisition
 ...
 * @prop {BillingInfoCreate} billingInfo
 ...
 * @prop {string} id - Optional, but if present allows an existing account to be used and updated as part of the purchase.
 * @prop {string} lastName
 * @prop {string} parentAccountCode - The account code of the parent account to be associated with this account. Passing an empty value removes any existing parent association from this account. If both `parent_account_code` and `parent_account_id` are passed, the non-blank value in `parent_account_id` will be used. Only one level of parent child relationship is allowed. You cannot assign a parent account that itself has a parent account.
 * @prop {string} parentAccountId - The UUID of the parent account to be associated with this account. Passing an empty value removes any existing parent association from this account. If both `parent_account_code` and `parent_account_id` are passed, the non-blank value in `parent_account_id` will be used. Only one level of parent child relationship is allowed. You cannot assign a parent account that itself has a parent account.
 * @prop {string} preferredLocale - Used to determine the language and locale of emails sent on behalf of the merchant to the customer. The list of locales is restricted to those the merchant has enabled on the site.
 ...
 */class AccountPurchase{static getSchema(){return{acquisition:'AccountAcquisitionUpdatable',address:'Address',billTo:String,billingInfo:'BillingInfoCreate',ccEmails:String,code:String,company:String,customFields:['CustomField'],email:String,exemptionCertificate:String,firstName:String,id:String,lastName:String,parentAccountCode:String,parentAccountId:String,preferredLocale:String,taxExempt:Boolean,transactionType:String,username:String,vatNumber:String};}}module.exports=AccountPurchase;