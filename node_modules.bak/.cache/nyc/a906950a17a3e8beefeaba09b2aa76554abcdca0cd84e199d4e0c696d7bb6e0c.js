 ...
 * @typedef {Object} SubscriptionAddOnUpdate
 ...
 * @prop {string} code - If a code is provided without an id, the subscription add-on attributes will be set to the current value for those attributes on the plan add-on unless provided in the request. If `add_on_source` is set to `plan_add_on` or left blank, then plan's add-on `code` should be used. If `add_on_source` is set to `item`, then the `code` from the associated item should be used.
 * @prop {string} id - When an id is provided, the existing subscription add-on attributes will persist unless overridden in the request.
 * @prop {number} quantity - Quantity
 * @prop {string} revenueScheduleType - Revenue schedule type
 * @prop {Array.<SubscriptionAddOnTier>} tiers - If the plan add-on's `tier_type` is `flat`, then `tiers` must be absent. The `tiers` object must include one to many tiers with `ending_quantity` and `unit_amount`. There must be one tier with an `ending_quantity` of 999999999 which is the default if not provided.
 * @prop {number} unitAmount - Optionally, override the add-on's default unit amount.
 ...
 */class SubscriptionAddOnUpdate extends Resource{static getSchema(){return{addOnSource:String,code:String,id:String,quantity:Number,revenueScheduleType:String,tiers:['SubscriptionAddOnTier'],unitAmount:Number,usagePercentage:Number};}}module.exports=SubscriptionAddOnUpdate;