...
class Pager {
   ...
   */
  async count () {
    const headers = await this.client._makeRequest('HEAD', this.path, null, this._consumeParams())
    const recordCount = headers['recurly-total-records']
    return recordCount && parseInt(recordCount, 10)
  }
  ...
  async first () {
    ...
    this.params['limit'] = 1
    let results = await this.client._makeRequest('GET', this.path, null, this._consumeParams())
