 ...
 * @prop {Date} redeemBy - The date and time the coupon will expire and can no longer be redeemed. Time is always 11:59:59, the end-of-day Pacific time. Null for bulk coupons configured with a relative redeem-by interval (see redeem_by_interval_unit and redeem_by_interval_amount).
 * @prop {number} redeemByIntervalAmount - For a bulk coupon with a relative redeem-by, the number of redeem_by_interval_unit intervals after a code's generation that it remains redeemable. Null unless the coupon uses a relative redeem-by.
 * @prop {string} redeemByIntervalUnit - For a bulk coupon with a relative redeem-by, the unit of the interval after which each generated unique code expires. Null unless the coupon uses a relative redeem-by.
...
class Coupon extends Resource {
  static getSchema () {
    return {
      ...
      plans: ['PlanMini'],
      redeemBy: Date,
      redeemByIntervalAmount: Number,
      redeemByIntervalUnit: String,
