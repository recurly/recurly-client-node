 ...
 * @prop {string} description - The description that gets sent to the gateway.
 ...
 * @prop {string} initiator - Must be sent for one-time transactions in order to provide context on which entity is submitting the transaction to ensure proper fraud checks are observed, such as 3DS. If the customer is in session, send `customer`. If this is a merchant initiated one-time transaction, send `merchant`.
 ...
 * @prop {TransactionNextAction} nextAction - Next action values are used for any required customer follow-up action. Currently, this is supported for Ebanx when using Pix Automatico.
 ...
 * @prop {Array.<PaymentGatewayReferences>} paymentGatewayReferences - Array of Payment Gateway References captured at transaction time, each a reference to a third-party gateway object of varying types.
...
class Transaction extends Resource {
  static getSchema () {
    return {
      ...
      nextAction: 'TransactionNextAction',
      object: String,
      origin: String,
      originalTransactionId: String,
      paymentGateway: 'TransactionPaymentGateway',
      paymentGatewayReferences: ['PaymentGatewayReferences'],
