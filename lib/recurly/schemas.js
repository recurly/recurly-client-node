      ...
      if (propertyType) {
        ...
        if (value === null) {
          ...
          // if it's not valid and we are in strict-mode, explode
            ...
            throw new Error(`${this.name} could not map property ${newKey} => ${inspect(value)} to schema type ${inspect(propertyType)}`)
          }
          // else we leave resource[newKey] undefined
          // this prevents the programmer from accidentally relying on it
        }
      } else if (utils.STRICT_MODE) {
        throw new Error(`${this.name} could not find schema property ${newKey} => ${inspect(value)}`)
      }
      // else we leave resource[newKey] undefined
      // this prevents the programmer from accidentally relying on it
    }
