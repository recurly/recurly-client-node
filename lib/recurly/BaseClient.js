    ...
    if (resp.status < 200 || resp.status > 299) {
      ...
      // based on the http status code
        ...
        if (resp.status === 400) {
          ...
          err = new networkErrors.ServiceUnavailableError('Recurly Service Unavailable Error', 'bad_gateway')
        } else if (resp.status === 503) {
          err = new networkErrors.ServiceUnavailableError('Recurly Service Unavailable Error', 'service_unavailable')
